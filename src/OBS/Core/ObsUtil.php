...
class ObsUtil
{
    const OBS_CONTENT = 'content';
    const OBS_LENGTH = 'length';
    const OBS_HEADERS = 'headers';
    const OBS_MAX_OBJECT_GROUP_VALUE = 1000;
    const OBS_MAX_PART_SIZE = 5368709120; // 5GB
    const OBS_MID_PART_SIZE = 10485760; // 10MB
    const OBS_MIN_PART_SIZE = 102400; // 100KB
    ...
    public static function validateOptions($options)
    {
        ...
        if ($options != NULL && !is_array($options)) {
            throw new ObsException ($options . ':' . 'option must be array');
        }
    }
    ...
    {
        if (empty($content)) {
            throw new ObsException("http body content is invalid");
        }
    }
...
BBB;
    ...
    {
        $content_md5 = "";
        if (($to_pos - $from_pos) > self::OBS_MAX_PART_SIZE) {
            return $content_md5;
        }
    ...
    public static function createDeleteObjectsXmlBody($objects, $quiet)
    {
        ...
        foreach ($objects as $object) {
            ...
            $object = ObsUtil::sReplace($object);
    ...
    public static function decodeKey($key, $encoding)
    {
        ...
        if ($encoding == "url") {
            ...
            throw new ObsException("Unrecognized encoding type: " . $encoding);
        }
    }
