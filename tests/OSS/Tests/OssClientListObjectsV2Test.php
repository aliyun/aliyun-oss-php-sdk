...
class OssClientListObjectsTest extends TestOssClientBase
{
    ...
    public function testListObjectsWithOwner()
    {
        $options2 = array(
            'delimiter' => '',
            'start-after' => 'folder/11',
        ...
        foreach ($listObjectInfo as $objectInfo) {
            $this->assertNotNull($objectInfo->getOwner());
            $this->assertNotNull($objectInfo->getOwner()->getId());
            $this->assertNotNull($objectInfo->getOwner()->getDisplayName());
        }
    }
