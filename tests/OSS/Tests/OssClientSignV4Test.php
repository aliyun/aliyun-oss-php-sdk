...
class OssClientSignV4Test extends TestOssClientBase
{
    /**
     * @var OssClient
     */
    ...
    public function testBaseInterfaceForObject()
    {
        ...
        $this->ossClient->setAdditionalHeaders(array('name', 'email'));
        try {
            $this->ossClient->uploadFile($this->bucket, $object, __FILE__, $options);
        } catch (OssException $e) {
            print_r($e->getMessage());
    ...
    public function testBaseInterfaceForBucektWithSts()
    {
        ...
        Common::waitMetaSync();
        $this->assertEquals($this->stsOssClient->getBucketAcl($this->bucket), OssClient::OSS_ACL_TYPE_PUBLIC_READ_WRITE, $options);
    ...
    {
        $ak = getenv('OSS_TEST_STS_ID');
        $sk = getenv('OSS_TEST_STS_KEY');
        $token = getenv('OSS_SESSION_TOKEN');
        if (empty($ak) || empty($sk) || empty($token)) {
